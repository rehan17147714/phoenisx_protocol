...
class PhoenixBrain {
  ...
  async answerQuery(input: string): Promise<Message> {
    ...
    if (deepSeekService.isApiConfigured()) {
      try {
        ...
        return {
          id: Date.now().toString(),
          type: 'phoenix',
          content: `🧠 **Phoenix AI (Powered by DeepSeek)**\n\n${deepSeekResponse}\n\n---\n*Powered by DeepSeek API • Phoenix Protocol v2.0*`,
          timestamp: new Date()
        };
      } catch (error) {
        console.error('DeepSeek API failed, falling back to advanced local responses:', error);
      }
    }
    ...
    // Advanced contextual responses for common patterns
