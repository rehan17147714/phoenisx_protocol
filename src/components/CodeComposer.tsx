...
enhanceWithAI(data) {
  ...
  const handleExecute = () => {
    ...
    phoenixBrain.executeCode(currentCode, language).then(result => {
  ...
  const handleAIAssist = () => {
    ...
    if (selectedAI === 'phoenix') {
      // Use DeepSeek for intelligent code assistance
      phoenixBrain.getCodeSuggestions(code, language).then(suggestions => {
        const aiResponse = `// 🧠 Phoenix AI Code Analysis (Powered by DeepSeek)
// Language: ${language.toUpperCase()}

${suggestions}

// Generated by Phoenix Protocol - DeepSeek Integration`;
        setNewCode(aiResponse);
      }).catch(error => {
        console.error('AI assist error:', error);
        setNewCode(`// ⚠️ AI assistance temporarily unavailable
...
// 4. Add proper documentation`);
      ...
      // Fallback for other AI assistants
